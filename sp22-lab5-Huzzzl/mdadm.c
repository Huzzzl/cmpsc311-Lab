...
int mdadm_mount(void) {  
  if (mo_counter == 0){
    mo_counter = 1;
    return jbod_client_operation(combine(JBOD_MOUNT,0,0),NULL);
    }
  return -1;
  }
...
int mdadm_unmount(void) { 
  if (mo_counter == 1){
    mo_counter = 0;
    jbod_client_operation(combine(JBOD_UNMOUNT,0,0),NULL);
    return 1;
    }
  return -1;
  }
...
int mdadm_read(uint32_t addr, uint32_t len, uint8_t *buf) {
  ...
  jbod_client_operation(combine(JBOD_SEEK_TO_DISK,disk,0),NULL);
  jbod_client_operation(combine(JBOD_SEEK_TO_BLOCK,0,block),NULL);
  ...
  while (address < end){
    ...
    jbod_client_operation(combine(JBOD_READ_BLOCK,0,0),temp);
  ...
  while (address < end){
    ...
    if (cache_enabled() && cache_lookup(disk,block,temp)==1){      
      if (end -address > 256){
        ...
        jbod_client_operation(combine(JBOD_WRITE_BLOCK,0,0),temp);
        } 
      else{
